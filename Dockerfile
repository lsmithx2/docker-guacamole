...
RUN useradd -u 99 -U -d /fsguac -s /bin/false fsguac                                                            && \
    usermod -G users fsguac                                                                                     && \
...
RUN rm -Rf /var/lib/tomcat9/webapps/ROOT                                                                                                                        && \
    ...
    chmod +x /bin/tini                                                                                                                                          && \
    mkdir -p /fsguac/guacamole /fsguac/log/tomcat9 /var/lib/tomcat9/temp /var/run/tomcat                                                                        && \
    ln -s /fsguac/guacamole /etc/guacamole                                                                                                                      && \
    rmdir /var/log/tomcat9                                                                                                                                      && \
    ln -s /fsguac/log/tomcat9 /var/log/tomcat9                                                                                                                  && \
...
### END
### To make this a persistent guacamole container, you must map /fsguac of this container
